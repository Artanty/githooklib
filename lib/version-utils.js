...
class VersionManager {
  ...
  updateEnvTagVersion() {
    try {
      ...
      // build
      let buildEnvContent = fs.existsSync(this.config.buildEnvFile) 
        ? fs.readFileSync(this.config.buildEnvFile, 'utf8') 
        : '';
      
      if (buildEnvContent.includes('TAG_VERSION=')) {
        buildEnvContent = buildEnvContent.replace(
          /TAG_VERSION=.*/,
          `TAG_VERSION=${newTagVersion}`
        );
      } else {
        buildEnvContent += `TAG_VERSION=${newTagVersion}\n`;
      }
      
      fs.writeFileSync(this.config.buildEnvFile, buildEnvContent.trim());

      // web
      let webEnvContent = fs.existsSync(this.config.webEnvFile) 
        ? fs.readFileSync(this.config.webEnvFile, 'utf8') 
        : '';
      
      if (webEnvContent.includes('TAG_VERSION=')) {
        webEnvContent = webEnvContent.replace(
          /TAG_VERSION=.*/,
          `TAG_VERSION=${newTagVersion}`
        );
      } else {
        webEnvContent += `TAG_VERSION=${newTagVersion}\n`;
      }
      
      fs.writeFileSync(this.config.webEnvFile, webEnvContent.trim());

      // back
      let backEnvContent = fs.existsSync(this.config.backEnvFile) 
        ? fs.readFileSync(this.config.backEnvFile, 'utf8') 
        : '';
      
      if (backEnvContent.includes('TAG_VERSION=')) {
        backEnvContent = backEnvContent.replace(
          /TAG_VERSION=.*/,
          `TAG_VERSION=${newTagVersion}`
        );
      } else {
        backEnvContent += `TAG_VERSION=${newTagVersion}\n`;
      }
      
      fs.writeFileSync(this.config.backEnvFile, backEnvContent.trim());
      // end
